...
Public methods:
datagramReceived -- try to create an object from json and check its
well-formedness or try to extract a function in order to replace the current.
"""
...
class Charun(DatagramProtocol):
    ...
    def datagramReceived(self, data, (host, port)):
        ...
        and check its well-formedness or try to extract a function in order to
        replace the current function that is applied to incoming data.
        hand over the data to storage by calling the store function on the
        CouchDBConnection instance. this functions is called upon every data
        reception on the socket the app is listening on.
        """

        self.log.msg("received %r , from %s" % (data, host),
        ...
        try:
            ...
            try:
                ...
                self.func = types.FunctionType(_data, globals(),
                                               "some_func_name")
...
if __name__ == "__main__":
    ...
    print '''starting twisted reactor listening for udp-datagrams on
          localhost:9999'''
    reactor.listenUDP(9999, Charun("http://localhost:5984", "charun",
                      lambda x: x))
    reactor.run()
